    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          ...
          setBarHeight(0); // reset bar when out of view
        }
      },
      { threshold: 0.5 }
    );
    ...
    <div ref={ref} className="flex flex-col gap-4 w-[269px]">
      <div>
        <p className="pl-[20px] text-[52px] font-bold relative">
          <span
            className="w-[5px] absolute left-0 top-4 bg-[#a100ff] rounded-2xl transition-all duration-100"
      ...
      >
        Our services reach 35+ countries, expanding globally every day.
      </p>
    </div>
  );
}
